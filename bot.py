...
class BotEventHandler(EventHandler):
    ...
    def get_response_text(self, the_event):
        ...
        sender_info = self.extract_sender_info(the_event)  # Function to extract sender info

        if the_event.kind == 'FOLLOW':
            response_text = f'hey {reply_name}, thanks for following! You were followed by: {sender_info}'
        elif the_event.kind == 'FOLLOW_BACK':
            response_text = f'hey {reply_name}, do you want to enter the raffle? Y or N. You were followed back by: {sender_info}'
        else:
